...
def split_data(affinity_df, random_seed, train_ratio=0.8, valid_ratio=0.1, test_ratio=0.1):
    ...
    test_df.to_csv(f'{args.output}/{random_seed}/test.csv', sep='\t', index=False)
    valid_df.to_csv(f'{args.output}/{random_seed}/valid.csv', sep='\t', index=False)
    train_df.to_csv(f'{args.output}/{random_seed}/train.csv', sep='\t', index=False)
    return test_df, valid_df, train_df

def write_hdf(input_df, set_name, failure_dict, element_dict, process_type, dataset_name, out_dir):
    print("\nfound {} complexes in {} dataset".format(len(input_df), set_name))
    with h5py.File(f'{out_dir}/{set_name}_{args.rec}.hdf', 'w') as f:
...
def main():
    ...
    if args.metadata:
        ...
        print(f'There are {len(affinity_data)} compounds in {args.dataset_name}, and will be randomly splitted into train/valid/test set(8/1/1) with 5 random seeds.')
        
        for random_seed in [111, 222, 333, 444, 555]:
            print(f'For random_seed {random_seed}.')
            failure_dict = {"name": [], "partition": [], "set": [], "error": []}
            out_dir = f'{args.output}/{random_seed}'
            if not os.path.exists(f'{out_dir}'):
                os.makedirs(f'{out_dir}') 
            test_df, valid_df, train_df = split_data(affinity_data, random_seed)
            train_name = f'{args.dataset_name}_train'
            failure_dict = write_hdf(train_df, train_name, failure_dict, element_dict, process_type, args.dataset_name, out_dir)
            valid_name = f'{args.dataset_name}_valid'
            failure_dict = write_hdf(valid_df, valid_name, failure_dict, element_dict, process_type, args.dataset_name, out_dir)
            test_name = f'{args.dataset_name}_test'
            failure_dict = write_hdf(test_df, test_name, failure_dict, element_dict, process_type, args.dataset_name, out_dir)
            failure_df = pd.DataFrame(failure_dict)
            failure_df.to_csv(f"{out_dir}/failure_summary.csv", index=False)
    ...
    if args.rec=='false_rec':
        ...
        for random_seed in [111, 222, 333, 444, 555]:
            ...
            if not os.path.exists(f'{out_dir}'):
                os.makedirs(f'{out_dir}') 
            index_dir = f'{args.output.split("/lig_alone")[0]}/complex_6A/{random_seed}'
            
            train_df_no_sort = pd.read_csv(f'{index_dir}/train.csv', sep = "\t")
            train_df = train_df_no_sort.sort_values(by=['-logAffi'])
            train_name = f'{args.dataset_name}_train'
            print(f'There are {len(train_df)} compounds in {train_name}.')
            failure_dict = write_hdf(train_df, train_name, failure_dict, element_dict, process_type, args.dataset_name, out_dir)

            valid_df_no_sort = pd.read_csv(f'{index_dir}/valid.csv', sep = "\t")
            valid_df = valid_df_no_sort.sort_values(by=['-logAffi'])
            valid_name = f'{args.dataset_name}_valid'
            print(f'There are {len(valid_df)} compounds in {valid_name}.')
            failure_dict = write_hdf(valid_df, valid_name, failure_dict, element_dict, process_type, args.dataset_name, out_dir)

            test_df_no_sort = pd.read_csv(f'{index_dir}/test.csv', sep = "\t")
            ...
            failure_df = pd.DataFrame(failure_dict)
            failure_df.to_csv("{}/failure_summary.csv".format(out_dir), index=False)

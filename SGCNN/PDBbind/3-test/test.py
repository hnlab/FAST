...
def test(args):
    ...
    output_f = "{}/{}.hdf".format(args.output, args.output_file_name) 
    ...
    fig, ax = plt.subplots()
    ax.set_title(f'{args.dataset_name}_test_on_core')
    ax.set_ylabel('predicted value')
    ax.set_xlabel('true value')
    ax.scatter(y, y_)
    ax.text(2,1,f'R2:{r2}; MAE:{mae}; pearsonr:{float(pearsonr[0])}; spearmanr:{float(spearmanr[0])}')
    fig.savefig(f'{args.output}/{args.output_file_name}.png', dpi=300)
    plt.close(fig)
...
if __name__ == "__main__": 
    ...
    parser.add_argument("--output-file-name", help="output file name", default='test_results')
